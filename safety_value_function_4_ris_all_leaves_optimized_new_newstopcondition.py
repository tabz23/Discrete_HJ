...
def _plot_classification_slice(
    ...
    if isinstance(env, DubinsCarEnvironment):
        obstacle = Circle(
            env.obstacle_position, env.obstacle_radius,
...
def run_algorithm_1(args):
    ...
    env = DubinsCarEnvironment(
        v_const=args.velocity, dt=args.dt, obstacle_radius=args.obstacle_radius
    )
    
    print(f"Environment: Dubins Car")
    print(f"  Velocity: {args.velocity}, Time step: {args.dt}")
    print(f"  Obstacle radius: {args.obstacle_radius}, Discount γ: {args.gamma}")
    ...
    value_iter = SafetyValueIterator(
        env=env, gamma=args.gamma, cell_tree=cell_tree, reachability=reachability
    )
...
def run_algorithm_2(args):
    ...
    print(f"Initializing coarse grid with resolution {args.initial_resolution}^3...")
...
def main():
    ...
    # Conservative parameters (Algorithm 3)
