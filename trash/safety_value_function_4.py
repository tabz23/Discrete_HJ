"""
Safety Value Functions with Formal Guarantees - PARALLELIZED Implementation
===========================================================================

Parallelization improvements:
1. Parallel Bellman updates using multiprocessing
2. Parallel successor set precomputation
3. Parallel ground truth computation
4. Efficient worker pools with proper resource management

Usage:
    python safety_value_function_parallel.py --algorithm 1 --resolution 10 --iterations 100 --workers 8
    python safety_value_function_parallel.py --algorithm 2 --epsilon 0.1 --refinements 5 --workers 8 --precompute
"""
...
# ============================================================================
# PART 4: PARALLEL VALUE ITERATION (ALGORITHM 1)
# ============================================================================

def _bellman_update_worker(args):
    """
    Worker function for parallel Bellman updates.
    
    Args:
        args: Tuple of (cell_data, gamma, env, reachability, all_leaves_data, successor_cache)
    
    Returns:
        Tuple of (cell_id, new_V_upper, new_V_lower)
    """
    cell_data, gamma, env, reachability, all_leaves_data, successor_cache = args
    
    # Reconstruct cell from data
    cell_id, bounds, center, V_upper, V_lower, l_upper, l_lower = cell_data
    
    # Reconstruct all leaves for successor computation
    all_leaves = {}
    all_leaves_list = []
    for leaf_data in all_leaves_data:
        leaf_id, leaf_bounds, leaf_center, leaf_V_upper, leaf_V_lower, _, _ = leaf_data
        leaf_cell = Cell(leaf_bounds, leaf_id)
        leaf_cell.center = leaf_center
        leaf_cell.V_upper = leaf_V_upper
        leaf_cell.V_lower = leaf_V_lower
        all_leaves[leaf_id] = leaf_cell
        all_leaves_list.append(leaf_cell)
    
    # Reconstruct current cell
    cell = Cell(bounds, cell_id)
    cell.center = center
    cell.V_upper = V_upper
    cell.V_lower = V_lower
    cell.l_upper = l_upper
    cell.l_lower = l_lower
    
    # Create temporary cell tree for successor computation
    class TempCellTree:
    ...
    temp_tree = TempCellTree(all_leaves_list)
    ...
    for action in env.get_action_space():
        ...
        cache_key = (cell_id, action)
        if successor_cache is not None and cache_key in successor_cache:
            successor_ids = successor_cache[cache_key]
            successors = [all_leaves[sid] for sid in successor_ids if sid in all_leaves]
        else:
            # ✅ FIX: Actually compute successors if not cached
        ...
        action_lower = gamma * min(lower_vals)
        action_upper = gamma * max(upper_vals)
    ...
    return (cell_id, new_V_upper, new_V_lower)
def _precompute_successor_worker(args):
    """
    Worker for parallel successor set precomputation.
    
    Args:
        args: Tuple of (cell_data, action, env, reachability, all_leaves_data)
    
    Returns:
        Tuple of ((cell_id, action), [successor_ids])
    """
...
class SafetyValueIterator:
    """Implements Algorithm 1 with parallelization."""
    ...
    def _precompute_all_successors(self):
        """Precompute all successor sets in parallel."""
        print(f"\nPrecomputing successor sets (parallel with {self.n_workers} workers)...")
        leaves = self.cell_tree.get_leaves()
        actions = self.env.get_action_space()
        
        # Serialize all leaves once
        all_leaves_data = [self._serialize_cell(cell) for cell in leaves]
        
        # Create tasks
        tasks = [
            (self._serialize_cell(cell), action, self.env, self.reachability, all_leaves_data)
            for cell in leaves
            for action in actions
        ]
        
        print(f"  Computing {len(tasks)} successor sets...")
        start_time = time.time()
        
        with Pool(self.n_workers) as pool:
            results = pool.map(_precompute_successor_worker, tasks)
        
        elapsed = time.time() - start_time
        
        # Store in cache
        self.successor_cache = {key: succs for key, succs in results}
    ...
    def value_iteration(self, max_iterations: int = 1000, convergence_tol: float = 1e-3,
        ...
        """Run value iteration until convergence (parallelized)."""
        ...
        print(f"\nStarting PARALLEL value iteration (max {max_iterations} iterations)...")
        ...
        for iteration in range(max_iterations):
            ...
            # Prepare tasks
            tasks = [
                (self._serialize_cell(cell), self.gamma, self.env, self.reachability,
                 all_leaves_data, self.successor_cache)
                for cell in leaves
            ]
            
            # Parallel Bellman updates
            with Pool(self.n_workers) as pool:
                results = pool.map(_bellman_update_worker, tasks)
            
            # Apply updates
            updates = {cell_id: (new_upper, new_lower) for cell_id, new_upper, new_lower in results}
            
            for cell in leaves:
                cell.V_upper, cell.V_lower = updates[cell.cell_id]
            
            # Check convergence
            diff_upper = max(abs(cell.V_upper - prev_upper[cell.cell_id]) for cell in leaves)
            diff_lower = max(abs(cell.V_lower - prev_lower[cell.cell_id]) for cell in leaves)
    ...
    def local_value_iteration(self, updated_cells: Set[Cell], max_iterations: int = 100,
        ...
        """LOCAL value iteration: only update BOUNDARY cells (parallelized)."""
        # Identify ALL boundary cells
        boundary_cells = set()
        for cell in self.cell_tree.get_leaves():
        ...
        # ✅ ALWAYS update successor cache for new cells (even if precompute is False)
        if updated_cells:
        ...
        for iteration in range(max_iterations):
            boundary_list = list(boundary_cells)
            prev_upper = {cell.cell_id: cell.V_upper for cell in boundary_list}
            prev_lower = {cell.cell_id: cell.V_lower for cell in boundary_list}
            
            # Serialize all leaves (needed for successor lookups)
            all_leaves_data = [self._serialize_cell(cell) for cell in self.cell_tree.get_leaves()]
            ...
            if self.n_workers > 1:
                ...
                results = [_bellman_update_worker(task) for task in tasks]
            ...
            diff_upper = max(abs(cell.V_upper - prev_upper[cell.cell_id]) for cell in boundary_list)
            diff_lower = max(abs(cell.V_lower - prev_lower[cell.cell_id]) for cell in boundary_list)
    ...
    def _update_successor_cache_for_new_cells(self, new_cells: Set[Cell]):
        """Update successor cache with new cells after refinement."""
        if not new_cells:
            return
        ...
        if self.n_workers > 1:
            ...
            results = [_precompute_successor_worker(task) for task in tasks]
        
        # Update cache
        for key, succs in results:
            self.successor_cache[key] = succs
...
class AdaptiveRefinement:
    """Algorithm 2/3 with parallelization."""
    ...
    def refine(self, epsilon: float, max_refinements: int = 100,
        ...
        """Main adaptive refinement loop with parallelization."""
...
def _simulate_trajectory_worker(args):
    """Worker for parallel ground truth trajectory simulation."""
    x, y, theta, env, time_horizon = args
...
def compute_ground_truth_reachability(
    ...
    """Compute ground truth safe set by forward simulation (parallelized)."""
    n_workers = n_workers or max(1, cpu_count() - 1)
    
    print(f"\n{'='*70}")
    print("COMPUTING GROUND TRUTH REACHABILITY (PARALLEL)")
    print(f"{'='*70}")
    ...
    total = len(tasks)
    print(f"\nSimulating {total} trajectories in parallel...")
    start_time = time.time()
    
    with Pool(n_workers) as pool:
        results_list = pool.map(_simulate_trajectory_worker, tasks)
    ...
    print(f"{'='*70}")
    print(f"Computation time: {elapsed:.2f}s ({total/elapsed:.1f} trajectories/s)")
...
def run_algorithm_1(args):
    """Runs Algorithm 1: Basic discretization with value iteration (parallelized)."""
    print("="*70)
    print("ALGORITHM 1: Discretization Routine (PARALLELIZED)")
    print("="*70)
...
def run_algorithm_2(args):
    """Runs Algorithm 2/3: Adaptive refinement (parallelized)."""
    print("="*70)
    print("ALGORITHM 2/3: Adaptive Refinement (PARALLELIZED)")
    print("="*70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Safety Value Function Computation with Formal Guarantees (PARALLELIZED)",
    ...
    parser.add_argument('--precompute', action='store_true',
                       help="Precompute all successor sets before VI (recommended for speed)")
    ...
    parser.add_argument('--plot-FT-HJ', action='store_true',
                       help="Generate ground truth reference via forward simulation")
